                    ...
                    <div className="ol-xl-9 col-lg-8 order-lg-2">
                        ...
                        <div class="tab-pane fade active show" id="tab-list" role="tabpanel" aria-labelledby="tab-shop-list">
                            ...
                            <nav className="course-single-bottom">
                                <ul class="nav course-tab" id="courseTab" role="tablist">
                                    {Post.map((item, index)=>{
                                    return <li className={hover == index?"nav-item active tip":"tip"} key={index} onMouseLeave={()=>handleLeave()} onMouseOver={()=>handleOver(index)} onClick={()=>handlemouse(index, item)}>
                                        <a className="lg:px-2 tip">{item.substr(0, 12)}</a>
                                        <span className={overon == index?"top-[-18px] absolute w-100 text-sm bg-[#A32926]  text-white":'hidden'} >{item.substr(0, 30)}</span>
                                    </li>
                                    })}
