...
let dataa = localStorage.getItem('Cart')?JSON.parse(localStorage.getItem('Cart')):[]
    ...
    const onApprove = (data, actions) => {
        ...
        return actions.order.capture().then((details) => {
            let transactionid = '';
            details.purchase_units.forEach((item) => {
                item.payments.captures.forEach((one) => {
            ...
            if (details.status && transactionid) {
                cart.forEach((item, index) => {
                    if (currencycode === 'NGN') {
                        ...
                        item.price = Math.round(item.price / convertnaira);
                        item.status = details.status;
                        item.code = transactionid;
                        item.moneyname = moneyname;
                    }
    ...
    const onError = (data, actions) => {
        // SetErrorMessage('your payment was not successful')
    };
