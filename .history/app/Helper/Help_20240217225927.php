...
class Help{
    ...
    public  function moneyconvert(){
      // dd((new Help)->getplace());
       ...
       $curl = curl_init();
       curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.fastforex.io/fetch-one?from=$from&to=$to&api_key=5bca86a903-93b4ec1d7e-s8kms0",
        CURLOPT_HTTPHEADER => array(
       "Accept: application/json"
        ),
         CURLOPT_RETURNTRANSFER => true,
    ...
    public  function constant(){
       ...
       $curl = curl_init();
       curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.fastforex.io/fetch-one?from=$from&to=$to&api_key=782ade6ad1-f9c6a09721-s90t2m",
        CURLOPT_HTTPHEADER => array(
